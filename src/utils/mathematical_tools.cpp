...
void NullSpaceProjectInplace(Eigen::MatrixXd& Hfx, int cols)
{
...
double CalcStereoDepth(const Eigen::Vector2d uv_left, const Eigen::Vector2d uv_right, const Eigen::Matrix3d K, const double baseline)
{
...
template <typename Derived>
Eigen::Matrix<typename Derived::Scalar, -1, -1> GivensRotation(const Eigen::MatrixBase<Derived>& mat, const int32_t stop_col)
{
    typedef typename Derived::Scalar Scalar_t;
