...
class Utils
{
   public:
    ...
    static void visualize_feature_tracking_results(const cv::Mat& image,
                                                   const std::pair<double, std::vector<CameraObs>>& frame_output,
                                                   const int wait_key_time_ms = 1,
                                                   cv::Mat* out_image = nullptr)
    {
        double fr = 11.333;
        double fg = 22.333;
        double fb = 33.333;
