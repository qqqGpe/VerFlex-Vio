...
{
    Eigen::Vector2d feature_norm(Kl_undistort_(0, 0) * p3d_norm.x() / p3d_norm.z() + Kl_undistort_(0, 2),
...
{
    Eigen::Vector2d feature_norm(Kr_undistort_(0, 0) * p3d_norm.x() / p3d_norm.z() + Kr_undistort_(0, 2),
...
void CameraModel::back_project_stereo(CameraObs& obs)
{
    ...
    Eigen::Vector3d feat_norm_right((obs.ur - Kr_undistort_(0, 2)) / Kr_undistort_(0, 0), (obs.vr - Kr_undistort_(1, 2)) / Kr_undistort_(1, 1), 1.0);
